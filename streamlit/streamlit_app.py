...
default_name_list = [
    "john smith, john wayne, lili peng, miguel garcia, lakisha johnson"
]
...
def app():
    ...
    Enter a list of (last) names or upload a CSV with (first and) last name columns. Here's a [sample CSV](https://raw.githubusercontent.com/appeler/ethnicolr/refs/heads/master/examples/input-with-header.csv)
    ...
    selected_function = st.sidebar.selectbox(
        label="Select a function", options=list(sidebar_options.keys())
    )

    if selected_function == "Append Census Data to Last Name":
        input_type = st.radio("Input type:", ("List", "CSV"))
        if input_type == "List":
            input_list = st.text_input(
                "Enter a list of last names (comma-separated):",
                value=", ".join(default_last_name_list),
            )
        ...
        if input_type == "List":
            ...
            if input_list:
                ...
                df = pd.DataFrame(list_name, columns=["lname_col"])
                lname_col = "lname_col"
                iter_val = st.sidebar.number_input(
                    "Enter number of iterations",
                    min_value=int_range[0],
                    max_value=int_range[1],
                    value=100,
                    step=1,
                )
                conf_int_val = st.sidebar.number_input(
                    "Enter confidence interval (0 to 1)",
                    min_value=float_range[0],
                    max_value=float_range[1],
                    value=0.9,
                    step=0.01,
                )
        ...
        function = sidebar_options[selected_function]
        if st.button("Run"):
            transformed_df = function(
                df, lname_col=lname_col, conf_int=conf_int_val, num_iter=iter_val
            )
            st.dataframe(transformed_df)
            download_file(transformed_df)
        ...
        if input_type == "List":
            input_list = st.text_input(
                "Enter a list of names (comma-separated):",
                value=", ".join(default_name_list),
            )
            if input_list:
                input_list = input_list.split(",")
                list_name = [x.strip() for x in input_list]
            ...
            if uploaded_file is not None:
                df = pd.read_csv(uploaded_file)
                st.write("Data loaded successfully!")
                fname_col = st.selectbox(
                    "Select column with the first names", df.columns
                )
                lname_col = st.selectbox(
                    "Select column with the last names", df.columns
                )
                iter_val = st.sidebar.number_input(
                    "Enter number of iterations",
                    min_value=int_range[0],
