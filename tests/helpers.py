...
def validate_probabilities_sum_to_one(df: pd.DataFrame, race_columns: list[str], tolerance: float = 0.1) -> bool:
    """
...
def validate_race_prediction_consistency(df: pd.DataFrame, true_race_col: str = "true_race",
                                       predicted_race_col: str = "race", min_accuracy: float = 0.6) -> bool:
    """
...
def validate_confidence_interval_columns(df: pd.DataFrame, mean_columns: list[str]) -> bool:
    """
    ...
    return validate_probability_ranges(df, mean_columns) and validate_no_nan_in_predictions(df, mean_columns)
...
def validate_basic_prediction_output(df: pd.DataFrame, model_type: str,
    ...
    if with_confidence:
        ...
        results.update({
            ...
            'confidence_probabilities_sum_to_one': validate_probabilities_sum_to_one(df, mean_columns),
            'confidence_no_nan': validate_no_nan_in_predictions(df, mean_columns),
            'confidence_valid_ranges': validate_probability_ranges(df, mean_columns),
        })
...
def assert_prediction_quality(df: pd.DataFrame, model_type: str, with_confidence: bool = False):
    """
    ...
    assert results['has_expected_columns'], f"Missing expected columns for {model_type} model"
    ...
    assert results['valid_probability_ranges'], "Found probability values outside [0,1] range"
    ...
    if with_confidence:
        ...
        assert results['confidence_probabilities_sum_to_one'], "Confidence probabilities do not sum to 1.0"
        assert results['confidence_no_nan'], "Found NaN values in confidence columns"
        assert results['confidence_valid_ranges'], "Found confidence values outside [0,1] range"


def create_test_summary(test_name: str, input_df: pd.DataFrame, output_df: pd.DataFrame,
                       model_type: str, with_confidence: bool = False) -> str:
    """
    ...
    """
    validation_results = validate_basic_prediction_output(output_df, model_type, with_confidence)
